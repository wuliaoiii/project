        ...
        $(function () {
            ...
            layui.use(['layer', 'form', 'layedit', 'laydate'], function () {
                ...
                form.on('submit(addeditsubmitfilter)', function (data) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "admin/provider/addupdateprovider",
        ...
        function pageInit() {
            ...
            jQuery("#list2").jqGrid(
                    {
                        ...
                        emptyrecords: "当前无记录",
                        colNames: ['ID', '供应商名称', '供应商地址', '手机号'],//jqGrid的列显示名字
                        colModel: [  //这里会根据index去解析jsonReader中root对象的属性，填充cell
                            {name: 'providerId', index: 'providerId', width: 100, sortable: true, search: false},
                            {
                            ...
                            {name: 'address', index: 'address', width: 200, sortable: false, search: false},
                        ...
                        prmNames: { //如当前查询实体为ware，这些可以在查询对象的superObject中设定
                            ...
                            id: "providerId", // 表示当在编辑数据模块中发送数据时，使用的id的名称
            ...
            $("#delete").click(function () {
                ...
                if (id) {
                    ...
                    layer.open({
                        content: '请确定是否真的要删除id为' + ret.providerId + '的供应商?',
                        ...
                        yes: function (index, layero) {//点击是时候的回调
                            ...
                            $.ajax({
                                type: "POST",
                                url: "admin/provider/deleteprovider",
                                data: {providerId: ret.providerId},
                                async: false,
                                error: function (request) {
                                    layer.alert("与服务器连接失败/(ㄒoㄒ)/~~");
                                },
            ...
            $("#edit").click(function () {
                ...
                if (id) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "admin/provider/selectproviderbyid",
                        data: {providerId: ret.providerId},
                        async: false,
                        error: function (request) {
                            layer.alert(data.mesg);
                        ...
                        success: function (data) {
                            ...
                            if (data.state == 'success') {
                                //向表单填充数据
                                $("#editlabelid").html(ret.providerId);//临时存放id，当提交时再去除赋值给input
                                $("#name").val(data.provider.name);
                                $("#address").val(data.provider.address);
                                $("#phone").val(data.provider.phone);
...
<div id="addeditformdivid" hidden="" class="layui-fluid" style="margin: 15px;">
    <form class="layui-form" action="" id="addeditformid">
        ...
        <input id="editid" name="providerId" value="" hidden="true"/>
        <div class="layui-form-item">
            <label class="layui-form-label">供应商名称</label>
            <div class="layui-input-block">
                <input type="text" id="name" name="name" lay-verify="userName" autocomplete="off" placeholder="请输入供应商名称"
                       class="layui-input">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" id="address" name="address" lay-verify="userName" autocomplete="off"
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" id="phone" name="phone" lay-verify="phone" autocomplete="off" placeholder="请输入手机号"
                       class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
