        ...
        function pageInit() {
            ...
            jQuery("#list2").jqGrid(
                    {
                        ...
                        emptyrecords: "当前无记录",
                        colNames: ['ID', '商品名称', '上次单价', '当前单价', '剩余库存', '最低库存'
                            , '图片地址', '条形码地址', '商品描述', '单位', '商品创建时间', '生产日期'
                            , '有效期限', '修改时间', '是否出售'],//jqGrid的列显示名字
                        colModel: [  //这里会根据index去解析jsonReader中root对象的属性，填充cell
                            {name: 'pid', index: 'pid', width: 100, sortable: true, search: false},
                            {
                                name: 'name', index: 'name', width: 180, sortable: false, search: true,
                                //被该列搜索时的搜索条件有哪些
                            ...
                            {name: 'lastPrice', index: 'lastPrice', width: 200, sortable: false, search: false},
                            {name: 'nowPrice', index: 'nowPrice', width: 180, sortable: false, search: false},
                            {name: 'leftCount', index: 'leftCount', width: 180, sortable: false, search: false},
                            {name: 'limitCount', index: 'limitCount', width: 180, sortable: false, search: false},
                            {name: 'picUrl', index: 'picUrl', width: 180, sortable: false, search: false},
                            {name: 'barCodeUrl', index: 'barCodeUrl', width: 180, sortable: false, search: false},
                            {name: 'des', index: 'des', width: 180, sortable: false, search: false},
                            {name: 'unit', index: 'unit', width: 180, sortable: false, search: false},
                            {
                                name: 'creationDate',
                                index: 'creationDate',
                                width: 180,
                            ...
                            {name: 'saleFlag', index: 'saleFlag', width: 180, sortable: false, search: false}
                        ],
                        ...
                        prmNames: { //如当前查询实体为ware，这些可以在查询对象的superObject中设定
                            ...
                            id: "pid", // 表示当在编辑数据模块中发送数据时，使用的id的名称
            ...
            $("#delete").click(function () {
                ...
                if (id) {
                    ...
                    layer.open({
                        content: '请确定是否真的要删除id为' + ret.pid + '的记录?',
            ...
            $("#edit").click(function () {
                ...
                if (id) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "admin/product/selectproductbyid",
                        data: {pid: ret.pid},
                        async: false,
                        error: function (request) {
                            layer.alert(data.mesg);
                        ...
                        success: function (data) {
                            ...
                            if (data.state == 'success') {
                                //向表单填充数据
                                $("#editlabelid").html(ret.pid);//临时存放id，当提交时再去除赋值给input
...
<div id="addeditformdivid" hidden="" class="layui-fluid" style="margin: 15px;">
    <form class="layui-form" action="" id="addeditformid">
        ...
        <input id="editid" name="pid" value="" hidden="true"/>
        ...
        <div class="layui-form-item">
            <label class="layui-form-label">上次单价</label>
            <div class="layui-input-block">
                <input type="text" id="lastPrice" name="lastPrice" lay-verify="isNum" autocomplete="off"
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" id="nowPrice" name="nowPrice" lay-verify="isNum" autocomplete="off"
                       placeholder="请输入单价" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="number" id="leftCount" name="leftCount" lay-verify="pNumber" autocomplete="off"
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="number" id="limitCount" name="limitCount" lay-verify="pNumber" autocomplete="off"
                       placeholder="请输入库存" class="layui-input">
            </div>
        </div>
        ...
        <#--<div class="layui-input-block">
            ...
            <div class="layui-input-block">
                <input type="text" id="picUrl" name="picUrl" lay-verify="notNull" autocomplete="off"
                       placeholder="请选择图片" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">条形码地址</label>
        <#--<div class="layui-input-block">
            <button type="button" class="layui-btn" id="barPicUpload">
                <i class="layui-icon">&#xe67c;</i>上传图片
            </button>
        </div>-->
            <div class="layui-input-block">
                <input type="text" id="barCodeUrl" name="barCodeUrl" lay-verify="notNull" autocomplete="off"
                       placeholder="请选择图片" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" id="des" name="des" lay-verify="notNull" autocomplete="off"
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" id="unit" name="unit" lay-verify="notNull" autocomplete="off"
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" id="productionDate" name="productionDate" lay-verify="notNull" autocomplete="on"
                       placeholder="请选择生产日期" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">有效期限</label>
            <div class="layui-input-block">
                <input type="text" id="expirationDate" name="expirationDate" lay-verify="notNull" autocomplete="on"
                       placeholder="请选择有效期限" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">是否出售</label>
            <div class="layui-input-block">
                <input type="radio" id="saleFlag" name="saleFlag" value="1" title="是" checked>
                <input type="radio" id="saleFlag" name="saleFlag" value="0" title="否">
            </div>
        </div>
        <div class="layui-form-item">
            <div class="layui-input-block">
