        ...
        $(function () {
            ...
            layui.use(['layer', 'form', 'layedit', 'laydate'], function () {
                ...
                form.on('submit(addeditsubmitfilter)', function (data) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "admin/account/addupdateaccount",
        ...
        function pageInit() {
            ...
            jQuery("#list2").jqGrid(
                    {
                        ...
                        emptyrecords: "当前无记录",
                        colNames: ['ID', '供应商主键', '账户', '账户类型', '使用标志', '创建时间'],//jqGrid的列显示名字
                        colModel: [  //这里会根据index去解析jsonReader中root对象的属性，填充cell
                            {name: 'aid', index: 'aid', width: 100, sortable: true, search: false},
                            {
                                name: 'providerId', index: 'providerId', width: 180, sortable: false, search: true,
                                //被该列搜索时的搜索条件有哪些
                            ...
                            {name: 'account', index: 'account', width: 200, sortable: false, search: false},
                            {name: 'type', index: 'type', width: 180, sortable: false, search: false},
                            {name: 'useFlag', index: 'useFlag', width: 180, sortable: false, search: false},
                            {
                                name: 'createDate',
                                index: 'createDate',
                                width: 180,
                        ...
                        prmNames: { //如当前查询实体为ware，这些可以在查询对象的superObject中设定
                            ...
                            id: "aid", // 表示当在编辑数据模块中发送数据时，使用的id的名称
            ...
            $("#delete").click(function () {
                ...
                if (id) {
                    ...
                    layer.open({
                        content: '请确定是否真的要删除id为' + ret.aid + '的记录?',
            ...
            $("#edit").click(function () {
                ...
                if (id) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "admin/account/selectaccountbyid",
                        data: {aid: ret.aid},
                        async: false,
                        error: function (request) {
                            layer.alert(data.mesg);
                        ...
                        success: function (data) {
                            ...
                            if (data.state == 'success') {
                                //向表单填充数据
                                $("#editlabelid").html(ret.aid);//临时存放id，当提交时再去除赋值给input
                                $("#providerId").val(data.account.providerId);
                                $("#account").val(data.account.account);
                                $("#type").val(data.account.type);
                                $("#useFlag").val(data.account.useFlag);
...
<div id="addeditformdivid" hidden="" class="layui-fluid" style="margin: 15px;">
    <form class="layui-form" action="" id="addeditformid">
        ...
        <input id="editid" name="aid" value="" hidden="true"/>
        <div class="layui-form-item">
            <label class="layui-form-label">供应商主键</label>
            <div class="layui-input-block">
                <input type="text" id="providerId" name="providerId" lay-verify="userName" autocomplete="off"
                       placeholder="请输入供应商主键"
                       class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" id="account" name="account" lay-verify="userName" autocomplete="off"
                       placeholder="请输入账户" class="layui-input">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="radio" id="type" name="type" value="0" title="微信" checked>
                <input type="radio" id="type" name="type" value="1" title="支付宝">
                <input type="radio" id="type" name="type" value="2" title="银联">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">使用标志</label>
            <div class="layui-input-block">
                <input type="radio" id="useFlag" name="useFlag" value="1" title="正常使用" checked>
                <input type="radio" id="useFlag" name="useFlag" value="0" title="不可使用">
            </div>
        </div>
        <div class="layui-form-item">
            <div class="layui-input-block">
